...
void *routine(void *arg) {
    ...
    bool (*model)(double, double, uint64_t) = (bool (*)(double, double, uint64_t)) parg.model;
    ...
    while (j < parg.y_len) {
        ...
        if (model(parg.x_range[i], parg.y_range[j], SIZE_MAT)) {
...
void *routine_single(void *arg) {
    ...
    fprintf(stderr, "id: %u, start: %lu, end: %lu\n\n", parg.thread_id, start, end);
    while (start < end) {
        for (uint64_t i = 1; i < 10; i++) {
            if (start == ist + istep*i) fprintf(stderr, "thread %u: %lu%%\n\n", parg.thread_id, i*10);
        }
        if (model(parg.x_range[start], SIZE_MAT)) parg.x_param_allowed->push_back(parg.x_range[start]);
        start++;
    }
    fprintf(stderr, "id: %u, len: %lu\n", parg.thread_id, parg.x_param_allowed->size());
    return NULL;
}
